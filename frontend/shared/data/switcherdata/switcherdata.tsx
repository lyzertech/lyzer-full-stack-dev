...
export const updateTheme = (themeType: any, clicked: string | boolean) => {
  const newState = {
    ...
    dataHeaderStyles:
      themeType === 'transparent' ? 'transparent' : 'transparent',
  ...
  setState(newState)
  // persist chosen theme and remove only the opposite explicit theme key
...
export const setPageStyle = (
  style: 'regular' | 'classic' | 'modern' | 'flat'
) => {
  const newState = {
    dataPageStyle: style,
  }
...
export const updateBackgroundColor = (
  ...
  // Persist the theme implied by background (use DEFAULT_THEME)
  localStorage.setItem(`vyzor${DEFAULT_THEME}Theme`, DEFAULT_THEME)
...
export const ThemePrimaryColor = () => {
  ...
  const [rgb, setRgb] = useState<{ colorPrimaryRgb: string }>({
    colorPrimaryRgb: '',
  })
...
export const Reset = () => {
  const newState = {
    lang: 'en',
    dir: 'ltr',
    dataThemeMode: DEFAULT_THEME,
    dataMenuStyles: DEFAULT_THEME === 'dark' ? 'dark' : 'light',
    dataNavLayout: 'vertical',
    dataHeaderStyles: DEFAULT_THEME === 'dark' ? 'dark' : 'light',
    dataVerticalStyle: 'doublemenu',
    toggled: '',
    dataNavStyle: '',
    horStyle: '',
  ...
  setState(newState)
  localStorage.clear()
  // Persist configured default theme selection
  localStorage.setItem(`vyzor${DEFAULT_THEME}Theme`, DEFAULT_THEME)
  if (DEFAULT_THEME === 'dark') {
    localStorage.removeItem('vyzorlightTheme')
  } else {
...
export const LocalStorageBackup = (setpageloading: any) => {
  ...
  localStorage.vyzornavstyles === 'menu-click'
    ? updateNavStyle('menu-click', 'menu-click-closed')
    : ''
  localStorage.vyzornavstyles === 'menu-hover'
    ? updateNavStyle('menu-hover', 'menu-hover-closed')
    : ''
  localStorage.vyzornavstyles === 'icon-click'
    ? updateNavStyle('icon-click', 'icon-click-closed')
    : ''
  localStorage.vyzornavstyles === 'icon-hover'
