...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const laravelRes = await fetch(`${LARAVEL_API_URL}/api/v1/finance/categories/${params.id}`, {
    ...
    return NextResponse.json({ error: 'Failed to fetch category', details: message }, { status: 500 })
  }
}
