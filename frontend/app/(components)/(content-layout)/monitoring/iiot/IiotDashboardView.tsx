...
interface IiotDashboardViewProps {}

const IiotDashboardView: React.FC<IiotDashboardViewProps> = () => {
  // State management
  const [isLive, setIsLive] = useState<boolean>(true)
  const [refreshRate, setRefreshRate] = useState<number>(1000)
  const [currentData, setCurrentData] = useState<ElectricalData>(
    generateElectricalData(),
  )
  ...
  const [voltageSparkline, setVoltageSparkline] = useState<number[]>(
    generateSparklineData(60),
  )
  const [currentSparkline, setCurrentSparkline] = useState<number[]>(
    generateSparklineData(60),
  )
  const [powerSparkline, setPowerSparkline] = useState<number[]>(
    generateSparklineData(60),
  )
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Update sparklines
      const avgVoltage =
        (newData.voltage.l1 + newData.voltage.l2 + newData.voltage.l3) / 3
  ...
  const handleExportCSV = useCallback(() => {
    if (dateRange.start && dateRange.end) {
      ...
      const exportData: ElectricalData[] = chartData.map((point, idx) => ({
        timestamp: new Date(
          Date.now() - (chartData.length - idx - 1) * refreshRate,
        ),
        voltage: {
          l1: point.l1,
          l2: point.l2,
  ...
  const avgVoltage =
    (currentData.voltage.l1 + currentData.voltage.l2 + currentData.voltage.l3) /
    3
...
export default IiotDashboardView
