...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const laravelRes = await fetch(`${LARAVEL_API_URL}/api/v1/finance/accounts/${params.id}`, {
    ...
    return NextResponse.json({ error: 'Failed to fetch account', details: message }, { status: 500 })
  }
}
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const message = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.json({ error: 'Failed to update account', details: message }, { status: 500 })
  }
}
