...
const DevicesPage = () => {
  ...
  const deviceStats = useMemo(() => {
    ...
    const offline = filteredDevices.filter(d => d.status === 'Offline' || d.status === 'Warning').length;
    const dormant = filteredDevices.filter(d => d.status === 'Inactive' || d.status === 'Dormant').length;
    
    // Assuming type parsing or using raw device type
    const electricity = filteredDevices.filter(d => ['Meter', 'Gateway', 'Sensor', 'Acuvim'].includes(d.device_type || '') || !d.device_type).length;
    const water = 0;
    const gas = 0;
    const activeAlerts = 0;

    return { total, online, offline, dormant, electricity, water, gas, activeAlerts };
  }, [filteredDevices]);
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Error: ' + (error.response?.data?.message || error.message || 'Failed to register device'))
    }
  }
  ...
  const handleBulkRegister = async () => {
    ...
    for (const item of toRegister) {
      try {
        ...
        errors.push(`${item.device_name}: ${err.response?.data?.message || err.message || 'Failed'}`)
      }
      done++
  ...
  return (
    <React.Fragment>
      ...
      <Row className="mb-4 gy-3">
        <Col xl={4} lg={12}>
          <Card className="custom-card shadow-sm border-0 h-100 overflow-hidden">
            <div className="card-bg-gradient" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #0dcaf0, #0d6efd)' }}></div>
            <Card.Header className="border-bottom-0 pb-1 pt-3">
              <Card.Title className="fw-bold fs-15 text-primary">Building Details</Card.Title>
            </Card.Header>
            <Card.Body className="d-flex align-items-center">
              <div className="me-4 rounded-3 p-3 bg-primary-transparent border border-primary-transparent d-flex align-items-center justify-content-center" style={{ width: '120px', height: '100px' }}>
                <i className="bi bi-building fs-1" style={{ color: 'var(--primary-color)' }}></i>
              </div>
              <div className="flex-fill">
                <div className="mb-2 d-flex align-items-center">
                  <i className="bi bi-buildings text-muted me-2 fs-14"></i>
                  <span className="text-muted fw-medium fs-13 me-2" style={{ width: '65px' }}>Name:</span>
                  <span className="fw-bold fs-13">{currentFacility?.name || 'All Facilities'}</span>
                </div>
                <div className="mb-2 d-flex align-items-center">
                  <i className="bi bi-geo-alt text-muted me-2 fs-14"></i>
                  <span className="text-muted fw-medium fs-13 me-2" style={{ width: '65px' }}>Location:</span>
                  <span className="fw-bold fs-13 text-info text-truncate" style={{ maxWidth: '140px' }} title={currentFacility?.full_address || currentFacility?.location_name || 'N/A'}>
                    {currentFacility?.full_address || currentFacility?.location_name || 'N/A'}
                  </span>
                </div>
                <div className="mb-2 d-flex align-items-center">
                  <i className="bi bi-bounding-box text-muted me-2 fs-14"></i>
                  <span className="text-muted fw-medium fs-13 me-2" style={{ width: '65px' }}>Area:</span>
                  <span className="fw-bold fs-13">{currentFacility?.metadata?.area || '1 sq. ft.'}</span>
                </div>
                <div className="d-flex align-items-center">
                  <i className="bi bi-grid text-muted me-2 fs-14"></i>
                  <span className="text-muted fw-medium fs-13 me-2" style={{ width: '65px' }}>Category:</span>
                  <span className="fw-bold fs-13">{currentFacility?.facility_type || 'Other'}</span>
                </div>
              </div>
            </Card.Body>
          </Card>
        ...
        <Col xl={4} lg={12}>
          <Card className="custom-card shadow-sm border-0 h-100 overflow-hidden">
            <div className="card-bg-gradient" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #ffc107, #fd7e14)' }}></div>
            <Card.Header className="border-bottom-0 pb-1 pt-3">
              <Card.Title className="fw-bold fs-15 text-warning">Weather Overview</Card.Title>
            </Card.Header>
            <Card.Body>
              <div className="d-flex align-items-center justify-content-between mb-4">
                <div className="d-flex align-items-center">
                  <i className="bi bi-cloud-sun fs-1 text-warning me-3" style={{ opacity: 0.8 }}></i>
                  <div>
                    <div className="fs-24 fw-bold text-dark d-flex align-items-center" style={{ color: 'var(--default-text-color)' }}>
                      29.6<span className="fs-18 text-muted ms-1">°C</span>
                    </div>
                    <div className="fs-12 text-muted fw-medium">Current Temperature</div>
                  </div>
                </div>
                <div className="text-end">
                  <div className="fs-12 fw-medium text-muted mb-1"><i className="bi bi-thermometer-high text-danger me-1"></i>High: <span className="fw-bold text-default">29.6°C</span></div>
                  <div className="fs-12 fw-medium text-muted"><i className="bi bi-thermometer-low text-info me-1"></i>Low: <span className="fw-bold text-default">29.6°C</span></div>
                </div>
              </div>
              <div className="bg-light p-3 rounded-3 d-flex justify-content-around align-items-center border border-default">
                <div className="text-center d-flex align-items-center">
                  ...
                  <div className="text-start">
                    <div className="fs-11 text-muted fw-bold text-uppercase">Feels Like</div>
                    <div className="fs-16 fw-bold">33°C</div>
                  </div>
                </div>
                <div style={{ width: '1px', height: '30px', background: 'var(--default-border)' }}></div>
                <div className="text-center d-flex align-items-center">
                  <i className="bi bi-droplet-fill text-primary fs-4 me-2"></i>
                  <div className="text-start">
                    <div className="fs-11 text-muted fw-bold text-uppercase">Humidity</div>
        ...
        <Col xl={4} lg={12}>
          <Card className="custom-card shadow-sm border-0 h-100 overflow-hidden">
            <div className="card-bg-gradient" style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '4px', background: 'linear-gradient(90deg, #198754, #20c997)' }}></div>
            <Card.Header className="border-bottom-0 pb-1 pt-3">
              <Card.Title className="fw-bold fs-15 text-success">Facility Device Overview</Card.Title>
            </Card.Header>
            <Card.Body className="p-3">
              <div className="d-flex align-items-start mb-3">
                <div className="me-3 p-2 bg-light rounded-3 text-center border border-default" style={{ minWidth: '80px' }}>
                  <div className="fs-22 fw-bold text-primary lh-1 mb-1">{deviceStats.total}</div>
                  <div className="fs-11 text-muted fw-medium">Total Devices</div>
                </div>
                <div className="flex-fill bg-light rounded-3 p-2 border border-default d-flex justify-content-between">
                  ...
                  <div className="flex-fill ms-3">
                    <div className="d-flex justify-content-between align-items-center mb-1">
                      <span className="fs-12 fw-medium text-muted"><span className="text-success me-1">●</span>Online</span>
                      <span className="fs-12 fw-bold">{deviceStats.online}</span>
                    </div>
                    <div className="d-flex justify-content-between align-items-center mb-1">
                      <span className="fs-12 fw-medium text-muted"><span className="text-warning me-1">▲</span>Offline</span>
                      <span className="fs-12 fw-bold">{deviceStats.offline}</span>
                    </div>
                    <div className="d-flex justify-content-between align-items-center">
                      <span className="fs-12 fw-medium text-muted"><span className="text-secondary me-1">●</span>Dormant</span>
                      <span className="fs-12 fw-bold">{deviceStats.dormant}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="d-flex gap-2">
                <div className="flex-fill bg-light rounded-3 p-2 border border-default">
                  <div className="d-flex justify-content-between mb-1 fs-12">
                    <span className="text-muted fw-medium"><i className="bi bi-lightning-charge-fill text-warning me-1"></i>Electricity</span>
                    <span className="fw-bold">{deviceStats.electricity}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-1 fs-12">
                    <span className="text-muted fw-medium"><i className="bi bi-droplet-fill text-info me-1"></i>Water</span>
                    <span className="fw-bold">{deviceStats.water}</span>
                  </div>
                  <div className="d-flex justify-content-between fs-12">
                    <span className="text-muted fw-medium"><i className="bi bi-fire text-danger me-1"></i>Gas</span>
                ...
                <div className="flex-fill bg-light rounded-3 p-2 border border-default">
                  <div className="d-flex justify-content-between mb-1 fs-12 border-bottom border-default pb-1">
                    <span className="text-muted fw-bold"><i className="bi bi-bell-fill text-warning me-1"></i>Active Alerts</span>
                    <span className="fw-bold">{deviceStats.activeAlerts}</span>
                  </div>
                  <div className="d-flex justify-content-between mb-1 fs-11 mt-1">
                    <span className="text-muted"><i className="bi bi-lightning-charge-fill text-warning me-1"></i>Electricity</span>
                    <span className="fw-bold">0</span>
                  </div>
                  <div className="d-flex justify-content-between mb-1 fs-11">
                    <span className="text-muted"><i className="bi bi-droplet-fill text-info me-1"></i>Water</span>
                    <span className="fw-bold">0</span>
                  </div>
                  <div className="d-flex justify-content-between fs-11">
                    <span className="text-muted"><i className="bi bi-fire text-danger me-1"></i>Gas</span>
                    <span className="fw-bold">0</span>
                  </div>
                </div>
              </div>
      ...
      <Row>
        <Col xl={12}>
          <FacilityEnergyChart facilityId={currentFacility ? String(currentFacility.id) : selectedOrg ? String(facilities[0]?.id || '') : null} />
        </Col>
      </Row>
                            ...
                            <div className="d-inline-flex gap-1">
                              ...
                              >
                                <Button
                                  ...
                                  size="sm"
                                  className="btn-icon rounded-pill shadow-sm border-0"
                                  onClick={() => router.push(`/monitoring/analysis?device_name=${device.name}`)}
                                >
                              ...
                              <OverlayTrigger
                                placement="top"
                                overlay={<Tooltip>View Monitoring Data</Tooltip>}
                              >
                                <Button
                                  variant="teal-light"
                                  size="sm"
      ...
      >
        ...
        <Modal.Body className="p-4">
          <Form id="device-form" onSubmit={handleFormSubmit}>
              ...
              <Row className="gy-3">
                ...
                <Col md={6}>
                  ...
                  <Form.Control
                    ...
                    onChange={(e) => setField('device_code', e.target.value)}
            ...
            <div className="p-3 rounded-3 border border-default bg-light mb-4">
              <p className="fw-bold fs-12 text-uppercase text-muted mb-3">
                <i className="bi bi-hdd-network me-1"></i> Connection & Protocol
              </p>
              <Row className="gy-3">
                <Col md={6}>
                  <Form.Label className="fw-semibold text-muted fs-11 text-uppercase">
                ...
                {/* MAC Address */}
                {['Ethernet', 'WiFi'].includes(formData.connection_type) && (
                  <Col md={6}>
                    <Form.Label className="fw-semibold text-muted fs-11 text-uppercase">
                      MAC Address
                    </Form.Label>
            ...
            {formData.protocol === 'OPC-UA' && (
              <div className="p-3 rounded-3 border border-primary-transparent bg-primary-transparent mb-4">
                <p className="fw-bold fs-12 text-uppercase text-primary mb-3">
                  <i className="bi bi-link-45deg me-1"></i> OPC-UA Parameters
                </p>
                <Row className="gy-3">
                  <Col md={12}>
                    <Form.Label className="fw-semibold text-muted fs-11 text-uppercase">
            ...
            {['HTTP', 'HTTPS'].includes(formData.protocol) && (
              <div className="p-3 rounded-3 border border-success-transparent bg-success-transparent mb-4">
                <p className="fw-bold fs-12 text-uppercase text-success mb-3">
                  <i className="bi bi-globe me-1"></i> HTTP / REST Parameters
                </p>
                <Row className="gy-3">
                  <Col md={12}>
                    <Form.Label className="fw-semibold text-muted fs-11 text-uppercase">
            ...
            {formData.connection_type === 'LoRaWAN' && (
              <div className="p-3 rounded-3 border border-warning-transparent bg-warning-transparent mb-4">
                <p className="fw-bold fs-12 text-uppercase text-warning mb-3">
                  <i className="bi bi-broadcast me-1"></i> LoRaWAN Parameters
                </p>
                <Row className="gy-3">
                  <Col md={6}>
                    <Form.Label className="fw-semibold text-muted fs-11 text-uppercase">
      ...
      >
        <Modal.Header closeButton className="border-0 pb-0">
          <Modal.Title className="fw-bold text-primary">
            <i className="bi bi-broadcast me-2"></i>Discovered Acuvim Devices
          </Modal.Title>
        </Modal.Header>
        <Modal.Body className="p-4">
          {/* ── Scanning spinner ── */}
          ...
          {bulkProgress && (
            ...
            >
              ...
              <ProgressBar
                ...
                variant={bulkProgress.errors.length > 0 ? 'warning' : 'success'}
                style={{ height: 4 }}
                className="mb-2"
              />
