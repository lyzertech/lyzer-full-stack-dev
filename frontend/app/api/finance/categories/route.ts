...
export async function GET(req: NextRequest) {
  try {
    ...
    // Support ?tree=1 to call the tree endpoint
    const isTree = searchParams.get('tree') === '1'
    searchParams.delete('tree')
    const endpoint = isTree ? 'categories/tree' : 'categories'
    const url = `${LARAVEL_API_URL}/api/v1/finance/${endpoint}?${searchParams.toString()}`
...
export async function POST(req: NextRequest) {
  try {
    ...
    const laravelRes = await fetch(`${LARAVEL_API_URL}/api/v1/finance/categories`, {
      method: 'POST',
      headers: {
