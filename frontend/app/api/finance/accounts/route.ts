...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `${LARAVEL_API_URL}/api/v1/finance/accounts?${searchParams.toString()}`
    ...
    return NextResponse.json({ error: 'Failed to fetch accounts', details: message }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const laravelRes = await fetch(`${LARAVEL_API_URL}/api/v1/finance/accounts`, {
      method: 'POST',
      headers: {
    ...
    const message = error instanceof Error ? error.message : 'Unknown error'
    return NextResponse.json({ error: 'Failed to create account', details: message }, { status: 500 })
  }
}
