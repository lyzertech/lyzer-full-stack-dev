...
const CreateEngineeringWikiPage: React.FC = () => {
  ...
  const [form, setForm] = useState<CreateEngineeringWikiInput>({
    title: '',
    customer_name: '',
    category: 'note',
    brand: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError(null)
    try {
      // Upload all files first and collect paths
      ...
      // Now create the wiki entry
      await createEngineeringWiki(updatedForm)
      router.push('/labs/engineering-wiki')
    } catch (err: any) {
      console.error(err)
      setError(err.message || 'Failed to create engineering wiki')
      setUploading(false)
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <Fragment>
      <Seo title="Add Engineering Wiki" />
      <Pageheader
        title="Labs"
        subtitle="Engineering Wiki"
        currentpage="Add Engineering Wiki"
        activepage="Add Engineering Wiki"
      />

      <Row className="mb-3">
        <Col xl={12}>
          <div className="d-flex justify-content-between align-items-center">
            <h4 className="fw-bold py-3 mb-0">Add Engineering Wiki</h4>
              ...
              <Form onSubmit={handleSubmit}>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <div className="input-group">
                        <Form.Select
                          id="brandSelect"
                          onChange={(e) =>
                            handleSelectChange('brand', e.target.value, 'brandInput')
                          }
                        >
                          <option value="">Select</option>
                          {brands.map((brand) => (
                        ...
                        <Form.Control
                          ...
                          value={form.brand || ''}
                          onChange={(e) =>
                            handleInputChange('brand', e.target.value, 'brandSelect')
                          }
                        />
                      </div>
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <div className="input-group">
                        <Form.Select
                          id="deviceTypeSelect"
                          onChange={(e) =>
                            handleSelectChange('device_type', e.target.value, 'deviceTypeInput')
                          }
                        >
                          <option value="">Select</option>
                        ...
                        <Form.Control
                          ...
                          onChange={(e) =>
                            handleInputChange('device_type', e.target.value, 'deviceTypeSelect')
                          }
                        />
                      </div>
                    </Form.Group>
                    ...
                    <Form.Group className="mb-3">
                      ...
                      <div className="input-group">
                        <Form.Select
                          id="modelSelect"
                          onChange={(e) =>
                            handleSelectChange('model', e.target.value, 'modelInput')
                          }
                        >
                          <option value="">Select</option>
                          {models.map((model) => (
                        ...
                        <Form.Control
                          ...
                          value={form.model || ''}
                          onChange={(e) =>
                            handleInputChange('model', e.target.value, 'modelSelect')
                          }
                        />
                      </div>
                    </Form.Group>
                ...
                <div className="mt-3 text-end">
                  <Button variant="primary" type="submit" disabled={submitting || uploading}>
                    {uploading ? 'Uploading files...' : submitting ? 'Saving...' : 'Save'}
                  </Button>
                </div>
              </Form>
            </Card.Body>
...
export default CreateEngineeringWikiPage
